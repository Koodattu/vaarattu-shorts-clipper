...
def test_search_uses_exact_id_and_never_enables_chat_from_a_title(monkeypatch):
    ...
    def handle(request):
        ...
        return httpx.Response(
            ...
            json={
                "data": {
                    "matches": [{"id": 9, "streamOffsetSeconds": 12000, "identity": "linked"}],
                    "suggestedStreamId": 9,
                }
            },
        )
...
def test_manual_stream_bypasses_search_even_for_old_streams_and_maps_split_offset(monkeypatch):
            ...
            return httpx.Response(
                ...
                json={
                    "data": {
                        ...
                        "points": [
                            {
                                "time": "2026-07-09T15:21:00Z",
                                "endTime": "2026-07-09T15:22:00Z",
                                "activeChatters": 20,
                            }
                        ],
                    }
    ...
    result = stream_data.enrich(
        {"title": "unrelated"}, {"stream_id": 1, "stream_offset_seconds": 12000, "alignment_confirmed": True}
    )
...
def test_older_backend_falls_back_without_confirming_ambiguous_identity(monkeypatch, duplicate):
    def handle(request):
        ...
        stream = {
            "id": 1,
            "startTime": "2026-07-08T22:00:00Z",
            "segments": [{"title": "hyvää päivää -> https://example.com"}],
        }
...
def test_local_lookup_validates_inputs_and_returns_suggestions(settings, monkeypatch):
    seen = []
    monkeypatch.setattr(
        stream_data, "search", lambda metadata: seen.append(metadata) or {"matches": [], "status": "ready"}
    )
    ...
    assert (
        client.get(
            "/api/streams/search", params={"video": "https://youtu.be/aaaaaaaaaaa", "q": "A title"}
        ).status_code
        == 200
    )
