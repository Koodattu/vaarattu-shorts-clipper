...
test("posting uses chosen daily settings and cleanup requires an inspected fingerprint and explicit confirmation",async()=>{
  ...
  assert.match(nodes.get("delivery-message").textContent,/does not upload automatically/);
