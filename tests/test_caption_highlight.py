...
def test_active_word_only_and_neutral_pause(tmp_path):
    ...
    assert len(events) == 3
    purple, white = r"{\1c&H00FC84C0&}", r"{\1c&H00FFFFFF&}"
    assert f"{purple}Hyvää{white} päivää" in events[0]
    assert "0:00:00.00,0:00:00.50" in events[0]
    assert purple not in events[1] and "0:00:00.50,0:00:00.70" in events[1]
    assert f"Hyvää {purple}päivää{white}" in events[2]
    assert all(r"{\fs86}" in event for event in events)
    assert purple not in (tmp_path / "captions.srt").read_text("utf-8")
...
def test_overlaps_have_one_phrase_and_one_highlight_at_a_time(tmp_path):
    ...
    assert all(event.count(r"{\1c&H00FC84C0&}") == 1 for event in events)
