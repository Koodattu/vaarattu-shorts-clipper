...
Choose **Concurrent jobs** on the new-run form to process 1–4 jobs at once (default 1). The preference is saved in the project database and takes effect immediately. Extra jobs wait in the queue; raising the limit starts waiting jobs, while lowering it lets active jobs finish before admitting more. Paused jobs do not occupy a processing slot. Each job still covers one explicitly selected video. The single worker coordinates these jobs; do not launch extra worker processes.
