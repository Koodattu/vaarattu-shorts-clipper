...
yt-dlp is installed with its official `default` dependencies, including the matching EJS challenge solver, and invoked with an isolated configuration and no playlist traversal. Install Deno (2.3+) or Node (22+) on PATH for YouTube extraction. The app keeps yt-dlp's default Deno selection when available; otherwise it explicitly enables an installed Node. See the [yt-dlp EJS guide](https://github.com/yt-dlp/yt-dlp/wiki/EJS). The app does not borrow browser cookies or a sibling project's extractor configuration. Warnings remain in the local diagnostic logs.

YouTube audio and video-section downloads refresh extraction after recognized temporary failures (HTTP 403/408/429/500/502/503/504 or transient connection failures). There are at most four attempts, separated by 30 seconds, 2 minutes and 5 minutes; pause/cancel remains responsive during these waits. If audio downloads keep returning 403, the last attempt requests an M4A audio stream in a separate file. Section downloads keep their original video/encoding settings. Unavailable/private recordings, unknown failures, time limits and size limits are not automatically retried. The run reports the recognized failure instead of only “external tool failed”. Each failed attempt has its own diagnostic log; `download-retry.json` records the attempts and next retry time in the media folder. Resuming a failed run starts a fresh bounded set of attempts. Metadata-only lookups and Twitch downloads retain their existing retry behavior.
...
Select a positive spend cap for API runs. Accounting uses conservative standard uncached rates, and peak rates for DeepSeek, so it can overestimate the provider's final invoice. Response usage includes reported reasoning where applicable. A timed-out or otherwise ambiguously billed request retains its reservation. There is one JSON repair attempt. Temporary model request failures (HTTP 408, 429, 500, 502, 503, 504, network errors, transport timeouts and remote protocol errors) automatically retry up to four times, waiting 30, 60, 120 and 240 seconds. The progress message shows a countdown, and pause/cancel remain available. A longer `Retry-After` is respected up to 15 minutes; longer requested waits stop for manual retry rather than retrying early. Access/configuration failures still stop immediately. Every retry reserves its own estimated cost under the existing cap and uses the same provider, model and request. The retry allowance is shared across the original request and its JSON repair. The application watchdog does not retry a request that may still be running. Exhaustion preserves completed work for Resume / retry. A run's cap is immutable; choose a sufficient cap initially or start a new run with a revised cap. There is no automatic paid fallback from local mode.
...
Choose **Concurrent jobs** on the new-run form to process 1–4 VOD jobs at once (default 1). The preference is saved in the project database and takes effect immediately. Extra VOD jobs wait in the queue; raising the limit starts waiting jobs, while lowering it lets active jobs finish before admitting more. Paused jobs do not occupy a processing slot. Each job still covers one explicitly selected video. The single worker coordinates these jobs; do not launch extra worker processes.

Review caption checks, tighter edit suggestions, context repairs and rerenders start independently of the VOD worker pool and concurrency setting. They also run alongside review actions for other recordings; there is no review worker-slot queue. Requests are still saved before dispatch so pause, resume and crash recovery work. The worker picks them up on its next polling cycle (normally within 0.2 seconds). Work on the same recording remains exclusive to protect its shared clip revisions and files. Full selection rechecks and bulk review-all rendering continue to use VOD slots.

Whisper and the local LLM take turns holding the GPU, unloading before another job can acquire it. A job waiting for the GPU remains running and can be paused/cancelled. Downloads, API/Codex requests and rendering can overlap across jobs. Start with 2 jobs when using API selection; local-only inference remains limited by the shared GPU. Token accounting and spending caps stay per run; concurrent runs increase aggregate provider usage and remain subject to provider/bridge limits.

Pause/cancel requests affect only their selected run and are handled at worker checks, including while external tools or model requests are in progress; completed chunks/windows remain saved. A running job releases its slot after cleanup. Resume uses matching checkpoints and joins the queue even if another run is active. Editing/rerendering a clip still requires that clip's own run to be inactive. Channel browsing is manual; there is no automatic channel-backlog selection, overnight schedule or publisher. Restart the launcher once to load this scheduler update; changing the concurrency setting afterwards needs no restart.
