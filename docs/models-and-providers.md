...
Updated 8 September 2026: **Turbo remains the full-VOD transcription model; large-v3 is an optional final pass for selected clips only.** The catalogue pins [Systran's faster-whisper large-v3 conversion](https://huggingface.co/Systran/faster-whisper-large-v3) to `edaa852ec7e145841d8ffdb056a99866b5f0a478`. Prepare it explicitly using [setup](setup.md), then enable **Refine selected clip captions with large-v3** for a new run. It uses the existing faster-whisper runtime, CUDA FP16, beam 5, fixed Finnish, word timestamps and unbatched decoding. It does not inherit Turbo's speed-oriented batch/Flash Attention settings. Downloaded weights and library caches remain project-local under `.cache/`. Other ASR comparisons below remain research. No large-v3 download, live inference or accuracy benchmark was performed during implementation.
