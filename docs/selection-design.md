# Transcript presentation and selection, conversation-v5
...
Coarse proposals of 5–90 seconds can reach verification, allowing correction of passage boundaries. Prefer 15–45 seconds and a concise worthwhile excerpt with enough context to understand it. Never pad to a minimum; a shorter complete joke can qualify. Final automatic selections must be 5–60 seconds, receive an accept judgment, score at least 3 for standalone, substance and fidelity, and have no flags. Opening and payoff contribute to ranking but have no acceptance threshold; there is no minimum total score that would indirectly require them. Five seconds is a technical sanity floor, not a prompting target. Refined spans must retain the original idea. The idea ID can advance within its original coarse passage to trim a preamble, but must stay in that passage and inside the final span; arbitrary moves to another idea passage are rejected. Export all non-overlapping passing clips, with no requested minimum or maximum count. Manual edits and saved older clips retain support up to 90 seconds. This is contiguous clipping, without sentence rearrangement or pause removal.
