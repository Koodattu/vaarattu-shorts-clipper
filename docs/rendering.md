...
- Camera height is adjustable from 192 to 1728 pixels; gameplay gets the rest of the 1920-pixel output. Existing presets default to the original 608/1312 split. Each panel can fill its area with a centered crop, or show the entire selection with black bars. Neither stretches the image.
- Select Camera or Gameplay, drag inside its crop to move it, and drag a corner to resize it. Arrow keys move one source pixel; Shift moves ten. Draw outside the existing crops or use **Redraw selected crop** to replace a selection; Escape cancels an active drag. Each crop supports freeform, output-panel shape, 16:9, 4:3, 1:1 and 9:16. The vertical preview uses the renderer's even-pixel crop rounding (scaler rounding can differ by a pixel). Use a frame from the actual recording without surrounding player borders.
- In **Edit**, pause the source player and click **Use this frame for layout**. This captures the video's own pixel area and loads that clip's saved crops. Adjust the camera and gameplay regions, check the preview, save the preset, then return to **Save and render revision**. The editor selects the clip's existing preset when opened and the newly saved preset after calibration.
- Saving with the same name replaces the preset, ignoring surrounding whitespace and letter case. Duplicate names from older saves collapse on replacement. **Delete preset** removes the preset and its screenshot after confirmation; existing runs and clips keep their stored composition snapshots. A different name creates a separate preset.
- Select or drop a screenshot to load it. Saving stores a compressed local JPEG copy and the original filename in the project database, separate from render snapshots; original dimensions are retained for crop validation and pixel snapping. Browsers do not expose the original disk path. Loading a preset restores its image, and the browser remembers the last selected preset across reloads. Old presets without an image need a screenshot once before saving.
- Captions use short phrases of up to four words / approximately 30 characters, balanced across at most two lines. Bold Arial has a base size of 86 (formerly 58), white text, bright lilac-purple (`#C084FC`) current-word highlighting, black outline and shadow. Unusually long lines reduce size using a conservative character-width estimate, not font measurement; long Finnish compounds still generate review warnings. SRT remains plain phrase captions, while ASS contains the highlighting. Word timestamps remain the source of truth; pauses inside a phrase are neutral. Overlaps use one visible phrase and at most one highlighted word at a time. ASS timing resolution is 10 ms; final output samples this at 30 fps.
...
Restart the app and reload the page to use the updated layout controls. Verification covers offline Python tests, Node geometry/editor checks, browser save/replace/restore and drag interactions, and a synthetic frame rendered through FFmpeg. No user media, inference, or existing runtime presets were used for these checks; verify your composition against its actual recording before rendering a revision.
