...
$("run-form").onsubmit=async event=>{event.preventDefault();if(submittingRun)return;submittingRun=true;error("");$("run-button").disabled=true;try{
  const body={video:$("video").value,asr:"turbo",provider:$("provider").value,local_model:$("local-model").value,context_size:Number($("context").value),budget_usd:$("provider").value==="codex"?0:Number($("budget").value),layout_id:$("layout").value,stream_id:$("stream-id").value?Number($("stream-id").value):null,stream_offset_seconds:$("stream-offset").value?Number($("stream-offset").value):null,alignment_confirmed:$("alignment").checked,discovery_reasoning:$("discovery-reasoning").value||"low",verification_reasoning:$("verification-reasoning").value||"low",video_encoder:$("video-encoder").value||"h264_nvenc",trim_silence:$("trim-silence").checked};
...
async function openEditor(id){$("editor-back").href="#results";$("editor-back").textContent="← Back to clips";editing=await api(`/api/clips/${id}`);const preset=savedLayouts.find(l=>sameLayout(l.body,editing.layout));if(preset)$("edit-layout").value=preset.id;else $("edit-layout").value="";$("use-source-frame").disabled=!editing.has_source;$("editor").hidden=false;$("editor-title").textContent=editing.title;$("edit-start").value=editing.start_us/1e6;$("edit-end").value=editing.end_us/1e6;$("edit-title").value=editing.title;$("edit-trim-silence").checked=Boolean(editing.trim_silence);$("edit-encoder").value=editing.video_encoder||"libx264";$("reviewed").checked=false;$("edit-words").value=editing.words.map(w=>`${w.id} | ${w.text}`).join("\n");$("source-player").hidden=!editing.has_source;if(editing.has_source){$("source-player").src=`/api/artifacts/${id}/source`;$("source-player").onloadedmetadata=()=>{$("source-player").currentTime=Math.max(0,(editing.start_us-editing.section_origin_us)/1e6-3);};}$("source-unavailable").hidden=editing.has_source;goView("editor");}
